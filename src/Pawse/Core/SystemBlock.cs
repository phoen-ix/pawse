...
/// <see cref="KeyboardFilterGuard"/> (Ctrl+Alt+Del + launch/media keys,
/// Enterprise/Education/IoT + admin). All three are opt-in
/// (<see cref="Config.SystemBlockCfg"/>) and default off.
///
/// <para><see cref="Apply"/> is the single entry point: it drives the full OS
/// state from the desired locked flag, so lock, unlock, the startup crash-sweep,
/// exit, and settings changes all funnel through it. The registry toggle runs
/// inline (fast); the WMI work is serialized off-thread so a slow round-trip never
/// stalls the caller.</para>
/// </summary>
public sealed class SystemBlock
{
    private static readonly string[] AllManagedIds =
        KeyboardFilterGuard.SecureAttentionIds
            .Concat(KeyboardFilterGuard.LaunchMediaIds)
            .ToArray();
    ...
    /// <summary>
    /// Reconcile the OS to the desired state. When <paramref name="locked"/> is
    /// true the enabled guards are applied; when false everything Pawse manages is
    /// reverted. Pass <paramref name="background"/> = false on exit so the revert
    /// completes before the process ends; true everywhere else.
    /// </summary>
    public void Apply(bool locked, bool background = true, bool notify = false)
    {
        // Each call supersedes the last: any still-queued background KF work from
        // an earlier call bails once it sees a newer generation.
        long gen = System.Threading.Interlocked.Increment(ref _generation);

        // Win+L - registry, fast, inline (Apply is always called on one thread, so
        // the last call's write wins naturally - no generation check needed here).
        if (locked && S.WinLock)
        {
    ...
    private string[] EnabledFilterIds()
    {
        ...
        if (S.SecureAttention) ids.AddRange(KeyboardFilterGuard.SecureAttentionIds);
        if (S.LaunchMediaKeys) ids.AddRange(KeyboardFilterGuard.LaunchMediaIds);
        return ids.ToArray();
    }
                ...
                if (!_kf.IsAvailable())
                {
                    if (notify && enableIds.Length > 0)
                        NotifyOnce("kf-unavailable", Elevation.IsElevated()
                            ? "Blocking Ctrl+Alt+Del / media keys needs the Windows Keyboard Filter " +
                              "feature (Enterprise, Education or IoT) - it isn't available on this PC."
                            : "Blocking Ctrl+Alt+Del / media keys needs Pawse running as administrator " +
                              "(tray menu → Restart as administrator), on Windows Enterprise, Education or IoT.");
                    return;
                }
                // Disable-then-enable, in order, so the final state is exact even
                // if a previous toggle is still settling.
