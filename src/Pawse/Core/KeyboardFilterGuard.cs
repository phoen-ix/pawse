...
/// <summary>
/// Blocks keys the user-mode hook fundamentally can't - <c>Ctrl+Alt+Del</c> (the
/// Secure Attention Sequence) and the browser / calculator / media / volume keys
/// (which bypass <c>WH_KEYBOARD_LL</c> by arriving as <c>WM_APPCOMMAND</c>) - using
/// the Windows <b>Keyboard Filter</b> feature via its WMI class
/// <c>WEKF_PredefinedKey</c> (namespace <c>root\standardcimv2\embedded</c>).
///
...
public sealed class KeyboardFilterGuard
{
    ...
    /// <summary>Ctrl+Alt+Del group. Win+L is belt-and-suspenders alongside the
    /// <see cref="WorkstationLock"/> policy toggle.</summary>
    public static readonly string[] SecureAttentionIds = { "Ctrl+Alt+Del", "Win+L" };
    ...
    private static bool Probe()
    {
        ...
        {
            Log.Warn("keyboard-filter: Pawse is not elevated - Ctrl+Alt+Del / media-key blocking unavailable");
            return false;
        }
        try
        {
